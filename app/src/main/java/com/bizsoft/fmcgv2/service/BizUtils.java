...
import com.bizsoft.fmcgv2.PaymentActivity;
...
public class BizUtils {
    ...
    public void showMenu(final Context context)
    {
        ...
        ImageButton sales = (ImageButton) dialog.findViewById(R.id.sales);
                    ...
                    if (newcustomer = true) {
                        ...
                        if (customers.getSale().size() > 0) {
                            ...
                            for (int y = 0; y < customers.getSalesOfCustomer().size(); y++) {
                                ...
                                new Save(context, "sale/save", "SDetails", customers.getSalesOfCustomer().get(y).getProducts(), customers.getId(), customers.getSalesOfCustomer().get(y), null, null,customers).execute();
           ...
           // System.out.println("JSON : " + json);
            ...
            if (url.toLowerCase().contains("order")) {
              ...
              //------------cheque details---------------
                ...
                //-----------------------------
                salex.setLedgerId(Store.getInstance().currentCustomerId.intValue());
                ...
                for(int i=0;i<Store.getInstance().transactionTypeList.size();i++)
                {
                    ...
                    if(Store.getInstance().transactionTypeList.get(i).getType().toLowerCase().contains(sale.getPaymentMode().toLowerCase()))
                    {
                        ...
                        salex.setTransactionTypeId(Store.getInstance().transactionTypeList.get(i).getId());
                    }
...
//                this.params.put("value", String.valueOf("1"));
    ...
    public void updateStock(SaleOrder saleOrder)
    {
        ...
        ArrayList<Product> currentProductsInCart = saleOrder.getProducts();
        for(int i=0;i<allProducts.size();i++)
        {
    ...
    public void showSyncDialog(final Context context)
    {
        ...
        builder.setMessage("Found Network Connectivity..Would you like to  sync ?")
                .setCancelable(false)
    ...
    public void sync(Context context,String from)
    {
        ...
        for (int i = 0; i < customerList.size(); i++) {
            ...
            if (customerList.get(i).getId() == null) {
                ...
                newcustomer = true;
                customerStatus = new SaveCustomer(context, customerList.get(i)).execute();
            }
        }
            ...
            if (customerList.get(i).getId() != null) {
                ...
                if (customerList.get(i).getSale().size() > 0) {
                    ...
                    for (int y = 0; y < customerList.get(i).getSalesOfCustomer().size(); y++) {
                        saleStatus = new Save(context, "sale/save", "SDetails", customerList.get(i).getSalesOfCustomer().get(y).getProducts(), customerList.get(i).getId(), customerList.get(i).getSalesOfCustomer().get(y), null, null,customerList.get(i)).execute();
                ...
                // new Save(context,"sale/save","SDetails",Store.getInstance().addedProductForSales).execute();
